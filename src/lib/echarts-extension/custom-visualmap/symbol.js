...
function symbolPathSetColor(color, innerColor, outlineColor, lineWidth) {
  const defaultLineWidth = 1
  if (this.type !== 'image') {
    var symbolStyle = this.style
    var symbolShape = this.shape
    if (symbolShape && symbolShape.symbolType === 'line') {
      symbolStyle.stroke = color || outlineColor
      symbolStyle.lineWidth = lineWidth || defaultLineWidth
    } else if (this.__isEmptyBrush) {
      symbolStyle.stroke = outlineColor || color
...
export function createSymbol(
  ...
  symbolPath.setColor(color, innerColor, outlineColor, lineWidth)
