...
;(function (root, factory) {
  ...
  function formatNumber(number, options, userLocale) {
    ...
    if (!maximumSignificantDigits) {
      ...
      if (fractionString.length < fractionDigits) {
        fractionString =
          fractionString + repeatZero(fractionDigits - fractionString.length)
      }
  ...
  function durationFormat() {
    ...
    var rawTokens = map(template.match(tokenizer), function (token, index) {
      ...
      return {
        ...
        token:
          type === 'escape' ? token.replace(tokenDefs.escape, '$1') : token,
    ...
    if (momentTypes.length > 1) {
      ...
      var bubbleTypes = function (bubble) {
        ...
        each(bubble.targets, function (target) {
          ...
          if (
            parseInt(bubbleMomentType.formattedValueEn, 10) === target.value
          ) {
            ...
            targetMomentType.formattedValueEn =
              targetMomentType.wholeValue.toString()
            bubbled = true
          }
        })
      }
  ...
  toLocaleStringRoundingWorks =
    toLocaleStringWorks && featureTestFormatterRounding(toLocaleStringFormatter)
