...
import { getStatsPointInfo } from '@/components/echarts/chart-options/gbinsar/utils'
...
export default {
  ...
  computed: {
    ...
    statsInfo() {
      return getStatsPointInfo(this.dataPoints)
    },
    filteredOptions() {
  ...
  methods: {
    async fetchData() {
      try {
        const response = await client.get('gbinsar/babadanpoint/', {
          params: {
        ...
        this.chartSeries = [
          ...
          {
            name: 'Kubah Lava 2021 Tumbuh',
            data: this.dataPoints.map((v) => [
              moment(v.timestamp).valueOf(),
              v.kubahlava2021tumbuh,
            ]),
            type: 'line',
          },
          {
