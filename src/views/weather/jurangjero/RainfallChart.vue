...
import {
  ...
  createSeries,
  mediaQuery,
} from '@/components/echarts/chart-options/weather-jurangjero'
...
export default {
  ...
  computed: {
    ...
    chartOptions() {
      const options = {
        baseOption: {
          ...baseChartOptions({ title: { text: 'Rainfall Jurang Jero' } }),
  ...
  methods: {
    ...
    async downloadData() {
      ...
      saveAs(
        blob,
        `weather-jurangjero-${createShortNameFromPeriod(this.period)}.csv`
      )
    },
  },
}
