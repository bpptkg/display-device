...
import {
  ...
  createSeries,
  mediaQuery,
} from '@/components/echarts/chart-options/weather-klatakan'
...
export default {
  ...
  computed: {
    ...
    chartOptions() {
      const options = {
        baseOption: {
          ...baseChartOptions({ title: { text: 'Rainfall Klatakan' } }),
  ...
  methods: {
    ...
    async downloadData() {
      ...
      saveAs(
        blob,
        `weather-klatakan-${createShortNameFromPeriod(this.period)}.csv`
      )
    },
  },
}
