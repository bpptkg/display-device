...
import {
  ...
  createSeries,
  mediaQuery,
} from '@/components/echarts/chart-options/weather-labuhan'
...
export default {
  ...
  computed: {
    ...
    chartOptions() {
      const options = {
        baseOption: {
          ...baseChartOptions({ title: { text: 'Rainfall Labuhan' } }),
  ...
  methods: {
    ...
    async downloadData() {
      ...
      saveAs(
        blob,
        `weather-labuhan-${createShortNameFromPeriod(this.period)}.csv`
      )
    },
  },
}
