...
export default {
  ...
  data() {
    return {
      ...
      station: 'jrakah_uvcamera',
      stationOptions: [
        {
          value: 'jrakah_uvcamera',
          text: 'Jrakah',
        },
      ],
      period: {
  ...
  methods: {
    async fetchData() {
      ...
      try {
        const response = await client.get('/uvcamera/', {
          params: {
    ...
    async downloadData() {
      ...
      saveAs(
        blob,
        `uvcamera-${this.station}-${createShortNameFromPeriod(this.period)}.csv`
      )
    },
