...
import fieldOptions from '@/store/thermal-axis-jrg/field-options'
import {
  BCard,
  BDropdownItem,
  BFormSelect,
...
export default {
  ...
  computed: {
    ...mapState({
      data(state) {
        return state.thermalAxisJrg[this.station].data
      },
      error(state) {
        return state.thermalAxisJrg[this.station].error
      },
      startTime(state) {
        return state.thermalAxisJrg[this.station].startTime
      },
      endTime(state) {
        return state.thermalAxisJrg[this.station].endTime
      },
      period(state) {
        return state.thermalAxisJrg[this.station].period
      },
      sampling(state) {
        return state.thermalAxisJrg[this.station].sampling
      },
      annotationOptions(state) {
        return state.thermalAxisJrg[this.station].annotationOptions
      },
      annotations(state) {
        return state.thermalAxisJrg[this.station].annotations
      },
      areas(state) {
        return state.thermalAxisJrg[this.station].areas
      },
      use_sky_filter(state) {
        return state.thermalAxisJrg[this.station].use_sky_filter
      },
      autoUpdate(state) {
        return state.thermalAxisJrg[this.station].autoUpdate
      },
    }),
    namespace() {
      return `thermalAxisJrg/${this.station}`
    },
    samplingType: {
      get: function () {
  ...
  methods: {
    ...
    async downloadData() {
      ...
      zip.generateAsync({ type: 'blob' }).then((content) => {
        saveAs(
          content,
          `thermal-axis-jrg-${this.station}-${createShortNameFromPeriod(
            this.period
          )}.zip`
        )
      })
