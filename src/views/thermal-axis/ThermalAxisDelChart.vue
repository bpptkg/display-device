...
import fieldOptions from '@/store/thermal-axis-del/field-options'
import {
  BCard,
  BDropdownItem,
  BFormSelect,
...
} from '@/store/thermal-axis-del/range-selector-day'
import rangeSelectorHour, {
  maxCustomDuration as maxCustomDurationHour,
} from '@/store/thermal-axis-del/range-selector-hour'
...
export default {
  ...
  computed: {
    ...mapState({
      data(state) {
        return state.thermalAxisDel[this.station].data
      },
      error(state) {
        return state.thermalAxisDel[this.station].error
      },
      startTime(state) {
        return state.thermalAxisDel[this.station].startTime
      },
      endTime(state) {
        return state.thermalAxisDel[this.station].endTime
      },
      period(state) {
        return state.thermalAxisDel[this.station].period
      },
      sampling(state) {
        return state.thermalAxisDel[this.station].sampling
      },
      annotationOptions(state) {
        return state.thermalAxisDel[this.station].annotationOptions
      },
      annotations(state) {
    ...
    namespace() {
      return `thermalAxisDel/${this.station}`
    },
    samplingType: {
      get: function () {
  ...
  methods: {
    ...
    async downloadData() {
      ...
      zip.generateAsync({ type: 'blob' }).then((content) => {
        saveAs(
          content,
          `thermal-axis-del-${this.station}-${createShortNameFromPeriod(
            this.period
          )}.zip`
        )
      })
