...
import fieldOptions from '@/store/thermal-axis-bbd/field-options'
import {
  BCard,
  BDropdownItem,
  BFormSelect,
...
export default {
  ...
  computed: {
    ...mapState({
      data(state) {
        return state.thermalAxisBbd[this.station].data
      },
      error(state) {
        return state.thermalAxisBbd[this.station].error
      },
      startTime(state) {
        return state.thermalAxisBbd[this.station].startTime
      },
      endTime(state) {
        return state.thermalAxisBbd[this.station].endTime
      },
      period(state) {
        return state.thermalAxisBbd[this.station].period
      },
      sampling(state) {
        return state.thermalAxisBbd[this.station].sampling
      },
      annotationOptions(state) {
        return state.thermalAxisBbd[this.station].annotationOptions
      },
      annotations(state) {
        return state.thermalAxisBbd[this.station].annotations
      },
      areas(state) {
        return state.thermalAxisBbd[this.station].areas
      },
      autoUpdate(state) {
        return state.thermalAxisBbd[this.station].autoUpdate
      },
    }),
    namespace() {
      return `thermalAxisBbd/${this.station}`
    },
    samplingType: {
      get: function () {
  ...
  methods: {
    ...
    async downloadData() {
      ...
      zip.generateAsync({ type: 'blob' }).then((content) => {
        saveAs(
          content,
          `thermal-axis-bbd-${this.station}-${createShortNameFromPeriod(
            this.period
          )}.zip`
        )
      })
