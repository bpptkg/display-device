 ...
 */
export const scaledFormatter = (units, unitFactor = 1000) => {
  if (unitFactor === 0) {
...
export const scaledSIFormatter = (unit = '', prefixOffset = 0) => {
  ...
  const units = symbols
    .slice(fractional.length + prefixOffset)
    .map((prefix) => {
      return `${prefix}${unit}`
    })
 ...
 */
export function axisLabelFormatter(value) {
  return scaledFormatter(['', 'k', 'M', 'G', 'T'])(value)
}
